...
namespace Seminar4
{
    class DeltaException : Exception
    {
        int delta;
        string mesaj;
        public DeltaException(int fdelta, string fmesaj) : base(fmesaj)
        {
            delta = fdelta;
    ...
    internal class Ec_gr2
    {
        ...
        public double radacina1 => (delta < 0) ? throw new DeltaException(delta, "Delta este negativ - nu exista radacini reale!") : (-b + Math.Sqrt(delta)) / (2 * a);
        ...
        public double radacina2
        {
            ...
            {
                if (delta < 0)
                    throw new DeltaException(delta, "Delta este negativ - nu exista radacini reale!");
                else
                    return ((-b - Math.Sqrt(delta)) / (2 * a));
            }
        }
